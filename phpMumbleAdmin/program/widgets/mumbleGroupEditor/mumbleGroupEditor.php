...
$PMA->popups->newHidden('channelGroupAdd');
...
if ($page->channelObj->id > 0) {
    $id = $page->channelsList[$page->channelObj->id]->parent;
...
if (is_object($widget->group)) {
    ...
    if ($widget->group->inherit) {
        ...
        $widget->group->inheritImg = PMA_IMG_SPACE_16;
    }
    // Inheritable
    if ($widget->group->inheritable) {
        $widget->group->inheritableImg = 'images/xchat/purple_16.png';
    } else {
        $widget->group->inheritableImg = PMA_IMG_SPACE_16;
    }
