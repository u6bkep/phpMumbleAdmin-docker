...
* Get datas from $prx
*/
$prx->getACL($page->channelObj->id, $aclList, $groupList, $inheritFromParent);
$registeredUsers = $prx->getRegisteredUsers('');
/*
* Create the default ACL.
*/
$default = PMA_MurmurObjectFactory::getAcl();
...
foreach ($aclList as $key => $acl) {
    ...
    */
    ...
    $acl->isSuperUserRu = ($page->channelObj->id === 0 && PMA_MurmurAclHelper::isSuperUserRuRule($acl));
    ...
    if (PMA_MurmurAclHelper::isToken($acl) OR PMA_MurmurAclHelper::isDenyAllToken($acl)) {
        $acl->img = PMA_IMG_MUMBLE_PW;
    } elseif ($acl->userid === -1) {
        $acl->img = PMA_IMG_GROUP_16;
    } else {
        $acl->img = PMA_IMG_MUMBLE_AUTH;
...
if ($inheritFromParent) {
    ...
    $widget->inheritImg = PMA_IMG_SPACE_16;
