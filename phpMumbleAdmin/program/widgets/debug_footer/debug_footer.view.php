...
if ($PMA->config->get('debug') > 0) {
    ...
    if ($PMA->config->get('debug_messages')) {
        ...
        foreach ($PMA->getDebugs() as $key => $array) {
...
<?php foreach ($widget->messages as $m): ?>
            <p>
                <span class="level">[<?= $m->lvl; ?>]</span>
<?php if ($m->class !== ''): ?>
                <span class="<?= $m->css; ?>"><?= $m->class; ?></span>
<?php endif;
if ($m->method !== ''): ?>
...
if (! empty($widget->session)): ?>
        <div class="debug">
            <h3>
                SESSION id:
                <mark class="sessid"><?= session_id(); ?></mark>
            </h3>
<?php foreach ($widget->session as $o): ?>
            <p>
                <span class="deep"><?= $o->deep; ?></span>
<?php if ($o->isArray): ?>
                <span class="array">[<?= $o->key; ?>]</span>
<?php else: ?>
