...
spl_autoload_register('PMA_autoload');
...
function pmaExceptionsOperations($e)
{
    ...
    if (substr($array['class'], 0, 7) === 'Murmur_') {
        ...
        switch ($array['class']) {
            ...
            * icesecretread will always be checked during ice connection
            * initialization.
            */
...
*/
function pmaGetGaugeFlag($nb, $max)
{
    $flag = 'null';
    ...
    if ($nb > 0) {
        if ($max === 0) {
            $flag = 'high';
        } elseif (($nb * 100 / $max) <= 70) {
            $flag = 'low';
        } elseif (($nb * 100 / $max) <= 90) {
            $flag = 'medium';
        } else {
            $flag = 'high';
        }
