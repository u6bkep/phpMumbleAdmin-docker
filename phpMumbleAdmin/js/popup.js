...
function unpop()
{
    if (el = document.getElementById(JsBgId)) {
        ...
        if (typeof(_PopupParent.getElementsByTagName('form')[0]) !== 'undefined') {
            _PopupParent.getElementsByTagName('form')[0].reset();
        }
        // Disable HTML href links, only if JSbackground has been created with createBackground() function .
        return false;
    }
}
...
{
    // Vertical center
    el.style.top = (document.documentElement.offsetHeight - el.offsetHeight)/2*0.85 +'px';
    // Horizontal center
